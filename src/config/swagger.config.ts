...
export default class SwaggerConfig {
  public static setup = (app: INestApplication): void => {
    ...
    const exportJson =
      configService.get<string>('EXPORT_SWAGGER_API_JSON') === 'true';
    if (exportJson) {
      ...
      if (existsSync(filePath)) {
        ...
        logger.log(
          'Swagger JSON export is disabled and no file exists to delete.',
        );
      }
    }
    ...
    SwaggerModule.setup('swagger', app, documentFactory, {
      ...
      jsonDocumentUrl:
        '/swagger-json' /* This is the default URL for the JSON document, but we can customize it if needed */,
      yamlDocumentUrl:
        '/swagger-yaml' /* This is the default URL for the YAML document, but we can customize it if needed */,
    });
  };
}
